...
// Si el usuario ya está en proceso de verificación, lo dejamos pasar
if (isset($_SESSION['asistente'])) {
  unset($_SESSION['asistente']); // eliminar sesión anterior
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$terminos) {
    ...
    // ===============================
    $stmt = $conn->prepare("SELECT usuario_id, email_verificado FROM usuarios WHERE email = ?");
    if (!$stmt) {
    ...
    if ($resultado->num_rows > 0) {
      ...
      if ($usuario['email_verificado'] == 1) {
        ...
        // ⚠️ Existe pero no ha verificado → reenviar a verificar
        ...
        header("Location: verificar-correo.php");
        exit;
      }
    } else {
      $stmt->close();
      ...
      if ($insertUsuario->execute()) {
        ...
        // ===============================
        // 🧩 GUARDAR DATOS TEMPORALES Y REDIRIGIR A VERIFICACIÓN
        // ===============================
        $_SESSION['verificar_correo'] = [
          'id' => $usuario_id,
          'correo' => $correo,
        ...
        // Enviar el primer código de verificación
        require_once "enviarCodigo.php";
...
<body>
  ...
  <main class="contenido">
    <div class="form-box">
      ...
      <p class="texto-login">
        <!-- (porque está en la misma carpeta el iniciar-asistente) -->
        ¿Ya tienes una cuenta? <a href="iniciar-asistente.php">Inicia sesión aquí</a>
      </p>
    </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-text">
          ...
          <ul>
            ...
            <li>💳 Una vez completada la compra, no se permiten devoluciones ni reembolsos, salvo cancelación del
              evento.</li>
            <li>📧 Aceptas recibir confirmaciones, recordatorios y actualizaciones del evento a través de correo
              electrónico.</li>
            <li>🚫 PUMFEST no se responsabiliza por la reventa no autorizada de entradas fuera de la plataforma oficial.
            </li>
